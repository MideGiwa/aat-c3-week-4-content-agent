...
// /api/auth/request-magic-link and /api/auth/login-password both have to be
// reachable with NO session — they're the calls the sign-in form itself
// makes, before any session exists. Leaving request-magic-link out
// (2026-09-18 bug) meant middleware redirected that POST to /login before
// it ever ran, fetch silently followed the redirect, and the client tried
// to JSON-parse an HTML page back — an uncaught parse error that left the
// sign-in button stuck on "Sending…" forever with no error ever shown.
// login-password would fail the exact same way if left off this list.
const PUBLIC_PATHS = [
  "/login",
  "/auth/callback",
