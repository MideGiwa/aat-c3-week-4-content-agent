...
n8n keeps exactly one job: the publish runner (`n8n-publish-workflow.json`), unchanged.
...
Scores the draft against the rubric — the PRD's default 9 criteria (Topic Relevance, Source Grounding, Factual Consistency, Audience Fit, Tone, SEO Fit, Channel Fit, Clarity, Completeness) plus any custom criteria configured for this request's channel/content type (`DESIGN.md` §13).
...
**Mock mode** (`evaluateDraft()`): scores are computed from the draft's own shape rather than a language model's judgment — a section with no cited source measurably lowers "Source Grounding" (the only score that isn't a static placeholder) and drives `status: "revise"`; a fully-cited draft scores `"pass"`. This is what makes the revision loop deterministic and testable rather than randomly sometimes triggering.
...
**On human approval:** all of this request's `draft`-status channel assets for the approved draft are promoted to `ready_to_publish` (`promoteChannelAssetsToReadyToPublish()` in `run.ts`) — that's what makes LinkedIn/X rows eligible for the n8n publish runner's query. Newsletter rows are promoted identically for consistency, even though nothing currently consumes `ready_to_publish` newsletter rows — they're reviewable and "ready" in exactly the same sense as the other channels, they just don't have a send mechanism yet.
